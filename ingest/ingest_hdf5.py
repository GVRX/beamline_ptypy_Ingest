...
def main():
    ...
    if dark is not None: 
        dark = orient(dark, rotate=args.rotate, flip=args.flip)

    if flat is not None: 
        flat = orient(flat, rotate=args.rotate, flip=args.flip)

    if mask is not None: 
        mask = orient(mask, rotate=args.rotate, flip=args.flip)
    ...
    if args.roi == 'auto':
        ...
        sy, sx = [int(v) for v in args.roi.split(',')]; target = (sy, sx)
